...
public static class Mouse {
    ...
    public static void ForceInit() {
        ...
        if (NtUserSetCursorPosBytes.Length == 0) {
            var temp = Misc.Help.GetUnmanagedFunctionBytes("win32u", "NtUserSetCursorPos");
            if (temp.Length > 0)
                NtUserSetCursorPosBytes = crypto.Encrypt(temp);
        }

        if (NtUserGetCursorPosBytes.Length == 0) {
            var temp = Misc.Help.GetUnmanagedFunctionBytes("win32u", "NtUserGetCursorPos");
            if (temp.Length > 0)
    ...
    #region SetCursorPos
    private static byte[] NtUserSetCursorPosBytes = [];
    ...
    public static bool SetCursorPos(int x, int y) {
        ...
        try {
            if (NtUserSetCursorPosBytes.Length == 0) {
                ...
                NtUserSetCursorPosBytes = Misc.Help.GetUnmanagedFunctionBytes("win32u", "NtUserSetCursorPos");
    ...
    private static Point<int> GetCursorPosW32() {
        if (!Native.User32.GetCursorPos(out var pt))
            return new Point<int>(-1, -1);

        return new Point<int>(pt.X, pt.Y);
    }

    private static byte[] NtUserGetCursorPosBytes = [];
    ...
    public static Point<int> GetCursorPos() {
        ...
        try {
            // same story
            if (NtUserGetCursorPosBytes.Length == 0) {
                NtUserGetCursorPosBytes = Misc.Help.GetUnmanagedFunctionBytes("win32u", "NtUserGetCursorPos");
